...
def generate_validation_set(x, label):
    index = int(label.shape[0] / 2)
    val_x = x[:index]
    val_y = np.zeros_like(label)[:index]

    index = int(label.shape[0] / 10)
    for i, letter in enumerate(test_str):
        row, col = np.where(str_matrix == letter)
        for j, lab in enumerate(label[i*index : (i+1)*index]):
            if lab == row+1 or lab == col+7:
                val_y[i*index+j] = 1
...
if __name__ == "__main__":
    ...
    val_x, val_y = generate_validation_set(test_x, test_label)
